...
if [ $# -lt 3 ]; then
    cat <<EOF
Usage:   transform_mni2func.sh [-n interp] [-a anat] [-p postmask] input output subject
...
if [ $ntp -gt 1 ]; then
    antsApplyTransforms -d 3 -e 3 -i $input -o $output -r $refvol -n $interp -t ${anat2func}.txt -t [${orig2temp},1] -t $temp2orig_warp
else
    antsApplyTransforms -d 3 -i $input -o $output -r $refvol -n $interp -t ${anat2func}.txt -t [${orig2temp},1] -t $temp2orig_warp
fi
