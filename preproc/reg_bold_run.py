...
# input files
srcvol = impath(srcdir, 'bold_cor_mcf_avg')
...
if args.refrun == args.runid:
    # just motion correct
    log.run('applywarp -i %s -r %s -o %s --premat=%s --interp=spline --rel --paddingsize=1' % (
        bold, refvol, bold_reg, mcf_file))
    ...
    # apply motion correction and affine co-registration
    log.run('applywarp -i %s -r %s -o %s --premat=%s --postmat=%s --interp=spline --rel --paddingsize=1' %
