...
onset files, confound matrix (if specified), the design matrix
generated by feat_model, and the input BOLD specified in the fsf
file. In the model, the first ntrials regressors are assumed to be the
trials of interest. Any additional regressors after that will be
included as regressors of no interest.

Specify the base for a design generated by FEAT. For example, if you
have a .fsf file in mydesign.fsf, specify mydesign as the
modelbase. Before running this script, you must run feat_model (for
example: feat_model mydesign) to generate the design matrix.
